...
/**
 * Fixed Step Raymarcher with BVH acceleration
 */
export class FixedStepBVH extends Raymarcher {
    public runRaymarcher(
        ...
        for (let y = yStart; y < yEnd; y++) {
            ...
            for (let x = 0; x < width; x++) {
                ...
                // --- perform BVH-accelerated fixed-step raymarch ---
                const depth = this.fixedStepMarchBVH(scene, rayOrigin, rayDir, idx, SDFevaluationBuffer, iterationsBuffer);
    ...
    private getNormal(scene: Scene, position: vec3, idx: number, SDFevaluationBuffer: Uint16Array): vec3 {
        ...
        const e = [0.01, 0, 0];
        const n = vec3.create();
        n[0] = d - this.getSceneDistanceBVH(scene, vec3.fromValues(position[0] - e[0], position[1], position[2]), idx, SDFevaluationBuffer);
        n[1] = d - this.getSceneDistanceBVH(scene, vec3.fromValues(position[0], position[1] - e[0], position[2]), idx, SDFevaluationBuffer);
        n[2] = d - this.getSceneDistanceBVH(scene, vec3.fromValues(position[0], position[1], position[2] - e[0]), idx, SDFevaluationBuffer);
        vec3.normalize(n, n);
        return n;
    }

    /**
     * BVH-accelerated fixed-step raymarch
     */
    private fixedStepMarchBVH(
        scene: Scene,
        rayOrigin: vec3,
        direction: vec3,
        idx: number,
